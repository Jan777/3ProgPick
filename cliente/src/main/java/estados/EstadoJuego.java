...
public class EstadoJuego extends Estado {
	...
	public void graficar(Graphics g) {
		if (juego.getPersonaje().getMapa() == 1) {
			g.drawImage(Recursos.mundo1, 0, 0, juego.getAncho(), juego.getAlto(), null);
		} else if (juego.getPersonaje().getMapa() == 2) {
			g.drawImage(Recursos.mundo2, 0, 0, juego.getAncho(), juego.getAlto(), null);
		} else {
			g.drawImage(Recursos.mundo3, 0, 0, juego.getAncho(), juego.getAlto(), null);
		}
		mundo.graficar(g);
	...
	private void graficarEstadoPersonaje(Graphics g) {
		...
		g.drawString(String.valueOf(paquetePersonaje.getSaludTope()) + " / "
				+ String.valueOf(paquetePersonaje.getSaludTope()), 132, 33);

		g.drawImage(Recursos.barraEnergia, 78, 41, ANCHOBARRA, ALTOENERGIA, null);
		g.drawString(String.valueOf(paquetePersonaje.getEnergiaTope()) + " / "
				+ String.valueOf(paquetePersonaje.getEnergiaTope()), 132, 53);

		if (paquetePersonaje.getExperiencia() == Personaje.tablaDeNiveles[paquetePersonaje.getNivel() + 1]) {
			drawBarra = ANCHOBARRA;
		} else {
			drawBarra = (paquetePersonaje.getExperiencia() * ANCHOBARRA)
					/ Personaje.tablaDeNiveles[paquetePersonaje.getNivel() + 1];
		}

		g.setFont(new Font("Tahoma", Font.PLAIN, 8));
		g.drawImage(Recursos.barraExperiencia, 77, 62, drawBarra, ALTOEXPERIENCIA, null);
		g.drawString(String.valueOf(paquetePersonaje.getExperiencia()) + " / "
				+ String.valueOf(Personaje.tablaDeNiveles[paquetePersonaje.getNivel() + 1]), 132, 68);
		g.setFont(new Font("Tahoma", Font.PLAIN, 10));
